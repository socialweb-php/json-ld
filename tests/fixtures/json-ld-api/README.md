> [!IMPORTANT]
> This directory holds only the `expand/`, `toRdf/`, `context.jsonld`,
> `expand-manifest.jsonld`, and `toRdf-manifest.jsonld` directories and files
> of the upstream test data from the [JSON-LD 1.1 Processing Algorithms and API
> Specification repository](https://github.com/w3c/json-ld-api). Other
> directories, files, and scripts mentioned in the text below are not vendored
> in this repository. See `NOTICE` at the repository root for the source commit
> and license.
