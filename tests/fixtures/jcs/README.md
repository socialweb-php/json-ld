> [!IMPORTANT]
> This directory holds only the `input/` and `output/` directories of the
> upstream test data from the [JSON Canonicalization Scheme reference
> implementation repository](https://github.com/cyberphone/json-canonicalization).
> The `outhex/` directory and the generator scripts mentioned in the text below
> are not vendored in this repository. See `NOTICE` at the repository root for
> the source commit and license.
